...
async function connectDB() {
  if (!productsCollection || !bidsCollection) {
    ...
    productsCollection = smartDealsDB.collection("products");
    bidsCollection = smartDealsDB.collection("bids");
...
app.get("/bids", verifyToken, async (req, res) => {
  ...
  try {
    const result = await bidsCollection.find({ buyerEmail: email }).toArray();
    res.send(result);
  } catch {
